...
bool Slave_recv::eventFilter(QObject* obj, QEvent* event)
{
	...
	else //不影响其他的控件的事件
	{
		...
		{
			if (list_widget->double_edit.double_port->isVisible())
			{
				list_widget->double_edit.double_ip->eventFilter(static_cast<QObject*>(list_widget->double_edit.double_ip), event);
				list_widget->double_edit.double_port->eventFilter(static_cast<QObject*>(list_widget->double_edit.double_port), event);
			}
		}
...
void Slave_recv::keyPressEvent(QKeyEvent * event)
{
	qDebug() << "Slave_recv::keyPressEvent ";
}